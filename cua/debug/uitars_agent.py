...
# NOTE Reference: https://github.com/xlang-ai/OSWorld/blob/main/mm_agents/prompts.py
...
class UITARSAgent:
    ...
    def predict(
        ...
        if self.observation_type in ["screenshot", "screenshot_a11y_tree", "a11y_tree"]:
            base64_image = obs["screenshot"] if self.observation_type in ["screenshot",
                                                                          "screenshot_a11y_tree"] else None
            try:
                linearized_accessibility_tree = (
        ...
        if len(self.history_images) > self.history_n:  # self.history_n = 5
