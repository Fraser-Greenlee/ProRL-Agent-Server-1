...
# Global semaphore to limit concurrent downloads from HuggingFace (avoids 429 rate limiting)
...
class SetupController:
    ...
    def _download_setup(self, files: List[Dict[str, str]]):
        ...
        for f in files:
            ...
            if not os.path.exists(cache_path):
                ...
                # Use semaphore to limit concurrent downloads (avoids 429)
                with _DOWNLOAD_SEMAPHORE:
                    for i in range(max_retries):
                        try:
                            backoff = min(2 ** i + 1, 60)  # exponential backoff: 2, 3, 5, 9, 17, 33, 60, 60
                            if i > 0:
                                logger.info(f"Waiting {backoff}s before retry {i+1}/{max_retries} for {url}")
                                time.sleep(backoff)
                ...
                if not downloaded:
                    raise requests.RequestException(f"Failed to download {url}. No retries left. Last error: {last_error}")
    ...
    def _chrome_open_tabs_setup(self, urls_to_open: List[str]):
        """Open Chrome tabs. Runs Playwright in a separate thread to avoid
        ...
        def _run_in_thread():
            try:
                result[0] = self._chrome_open_tabs_impl(urls_to_open)
            except Exception as e:
                error[0] = e

        # If we are inside an asyncio event loop, run Playwright in a clean thread
        import asyncio
        try:
            asyncio.get_running_loop()
            # We are inside an event loop - must use a thread
            t = threading.Thread(target=_run_in_thread)
            t.start()
            t.join(timeout=300)  # 5 min max
            if t.is_alive():
                raise TimeoutError("Chrome tab setup timed out")
            if error[0]:
                raise error[0]
            return result[0]
        except RuntimeError:
            # No event loop running - safe to call directly
            return self._chrome_open_tabs_impl(urls_to_open)

    def _chrome_open_tabs_impl(self, urls_to_open: List[str]):
        """Internal implementation of Chrome tab opening using sync Playwright."""
        host = self.vm_ip
    ...
    def _chrome_close_tabs_setup(self, urls_to_close: List[str]):
        """Close Chrome tabs. Runs Playwright in a separate thread to avoid
        conflicts with asyncio event loops."""
        result = [None]
        error = [None]
        ...
        def _run_in_thread():
            try:
                result[0] = self._chrome_close_tabs_impl(urls_to_close)
            except Exception as e:
                error[0] = e
        ...
        try:
            ...
            t.join(timeout=300)
            if t.is_alive():
                raise TimeoutError("Chrome close tabs timed out")
            if error[0]:
                raise error[0]
            return result[0]
        except RuntimeError:
            return self._chrome_close_tabs_impl(urls_to_close)

    def _chrome_close_tabs_impl(self, urls_to_close: List[str]):
        """Internal implementation of Chrome tab closing using sync Playwright."""
