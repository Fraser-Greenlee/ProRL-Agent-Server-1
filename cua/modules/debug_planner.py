...
logger = openhands_logger.getChild('planner')


class Planner:
    """
    Planner class that generates high-level goal & sub-goals
    """
    def __init__(self, args: Namespace):
        self.client = OpenAI(
            base_url=f"http://{args.planner_node}:8000/v1",
            api_key="gen",
        )
        self.model_name = args.planner_model_name
    ...
    def generate_subgoal(
        ...
        """
        Generate sub-goal conditioned on the high-level goal, and the sub-goals so far.
        Output consists of (1) the new sub-goal, (2) intent behind the new sub-goal.
        """
