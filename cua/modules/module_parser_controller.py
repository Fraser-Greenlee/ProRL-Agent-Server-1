...
class ParserController:
    ...
    def parse_screenshot(self, image_bytes: bytes) -> bytes:
        """
        ...
        # sub-sample bboxes (too many boxes can confuse the VLM)
        result["boxes"] = random.sample(result["boxes"], min(len(result["boxes"]), 30))
        parsed_image_bytes = self.prepare_image_with_som(image_bytes, result["boxes"])

        return parsed_image_bytes

    @staticmethod
    def prepare_image_with_som(image: Image.Image | bytes, bbox_list: List) -> bytes:
        """
