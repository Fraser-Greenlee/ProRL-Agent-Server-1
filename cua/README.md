...
for `/dev/kvm` access. \
(2) When the node is online with enroot container running, we ssh into the node, and fetch the enroot container ID from
the CPU node. \
...
This is a very finicky detail, but when the enroot container environment in step (1) loses `/dev/kvm` access that the
CPU node originally had. The only way to retain `/dev/kvm` access inside an enroot container is to first boot it up,
