...
Select the appropriate action using the osworld tool."""
    ...
    async def collect_trajectory(
        ...
        trajectory['steps'].append(initial_step_data)
    ...
    def submit_trajectory_job(self, trajectory_idx: int) -> str:
        ...
        job_id = f"job_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{trajectory_idx:03d}_{rand_num}"
        trajectory_id = f"trajectory_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{trajectory_idx:03d}_{rand_num}"
...
async def main():
    ...
    parser.add_argument(
        '--vm-image',
        type=str,
        default='/lustre/fsw/portfolios/nvr/users/yidong/Projects/ProRL-Agent-Server/OS_images/Ubuntu.qcow2',
        help='Path to VM image file'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default='./trajectories_new_1',
    ...
    parser.add_argument(
        '--llm-model',
        type=str,
        default='/lustre/fsw/portfolios/nvr/users/yidong/data/models/openai/gpt-oss-120b/',
        help='LLM model name'
    )
    parser.add_argument(
        '--max-steps',
    ...
    output_dir = os.path.join(output_dir, f"job_{args.job_index:03d}")
