...
def pre_process_r2egym_instance(r2egym_instance):
    ...
    r2egym_instance = r2egym_instance.apply(lambda x: x.tolist() if isinstance(x, np.ndarray) else x)
...
def test_server(
    ...
    r2egym_cur = r2egym_instance.copy(deep=True)
    r2egym_cur['trajectory_id'] = i+1
    requests.append(r2egym_cur.to_dict())
...
if __name__ == '__main__':
    start = time.time()
    results = test_server(total_jobs=1, max_parallel_jobs=1, allow_skip_eval=False)
