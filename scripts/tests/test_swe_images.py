    ...
    async def _evaluate_single(idx: int, inst: dict):
        async with semaphore:
            try:
                ...
                rep = await _evaluate_agent(patch, pd.Series(inst), sid=f"gold_{idx}", allow_skip=allow_skip)
...
def main():
    ...
    for idx, row in dataset_df.iterrows():
        ...
        inst_series = inst_series.apply(lambda x: x.tolist() if isinstance(x, np.ndarray) else x)
        if "trajectory_id" not in inst_series or pd.isna(inst_series.get("trajectory_id")):
            inst_series["trajectory_id"] = idx
        instances.append(inst_series.to_dict())

    start_ts = time.time()
    results = asyncio.run(
        _evaluate_instances(instances, concurrency=args.concurrency, allow_skip=args.allow_skip_eval)
    )
    ...
    print(f"[evaluate_gold] Resolved {resolved_cnt}/{len(results)} instances ({resolved_cnt/len(results):.2%})")
