#!/usr/bin/env bash
# Test OpenCode via the exact harness command that polar generates.
# Prerequisites: gateway at :8100, opencode CLI installed, polar importable
set -euo pipefail
...
curl -sf "$PROXY/sessions" \
  ...
  -d "{\"api_type\":\"openai_chat\",\"task_id\":\"test-opencode-polar\",\"session_id\":\"$SESSION_ID\"}" \
  > /dev/null
...
steps = OpenCodeHarness(spec).run_steps('''$PROMPT''')
